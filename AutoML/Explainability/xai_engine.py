...
class XAIEngine:
    ...
    def _create_explainer(self):
        ...
        try:
            # Try tree-based models (XGBoost, LightGBM, CatBoost, sklearn trees)
            if isinstance(self.model, tree_based_models) or \
               'XGB' in model_type or 'LGB' in model_type or 'CatBoost' in model_type:
                return shap.TreeExplainer(self.model)
        except Exception:
            pass
        
        try:
            # Try linear models (LogisticRegression, Ridge, Lasso, LinearRegression, etc.)
            if 'Linear' in model_type or 'Logistic' in model_type or \
               'Ridge' in model_type or 'Lasso' in model_type or 'ElasticNet' in model_type:
        ...
        background_data = shap.sample(self.X_train, min(100, len(self.X_train)))
    ...
    def save_global_explanation(self, path="shap_global.png"):
        ...
        try:
            ...
            # Fallback to bar plot if summary plot fails
            print(f"Summary plot failed: {e}, using bar plot instead")
            mean_abs_shap = np.abs(shap_values).mean(axis=0)
            feature_names = self.X_test.columns if hasattr(self.X_test, 'columns') else [f"Feature {i}" for i in range(shap_values.shape[1])]
            plt.figure(figsize=(10, 6))
            plt.barh(range(len(mean_abs_shap)), mean_abs_shap)
            plt.yticks(range(len(mean_abs_shap)), feature_names)
